...
(function () {
  ...
  rerun = create('div', {
    style: styles.rerun
  }), memRun = function memRun() {}, sec2time = function sec2time(sec) {
    var h = ~~(sec / 3600),
      m = ~~(sec % 3600 / 60),
      s = ~~(sec % 60);
  ...
  }, schedule = function schedule(config, setters, show, complete) {
    ...
    function runSchedule(_x4, _x5, _x6, _x7) {
    ...
    return runSchedule;
  }(function (schedules, index, setters, complete) {
    setTimeout(beep, 100);
    ...
    countdown(function () {
      schedules.length > index + 1 ? runSchedule(schedules, index + 1, setters, complete) : complete();
